...
impl CdkLdkNodeBuilder {
    ...
    pub fn build(self,store: Option<Arc<dyn KVStore + Sync + Send>>) -> Result<CdkLdkNode, Error> {
        CdkLdkNode::new(self,store)
    }
}
...
impl CdkLdkNode {
    ...
    pub fn new(
        ...
        let node = match store {
            ...
            Some(store_ref) => ldk.build_with_store(store_ref)?,
        };
